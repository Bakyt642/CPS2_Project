...
void sendValues() {
  ...
  if ( mqtt_client.connected() ) {
    if ( lum != -1 )
      mqtt_client.publish(String(topic +"/LUMI" ).c_str(), String("{\"Illumination \":\"")+String(lum).c_str()+String("\",")+String("\"Time \":\"")+String(dateTime)+String("\"}") );
    if ( tmp != -1 )
      mqtt_client.publish(String(topic+"/TEMP").c_str(), String("{\"Temperature *C \":\"")+String(tmp).c_str()+String("\",")+String("\"Time \":\"")+String(dateTime)+String("\"}")) ;
    if ( hmdt != -1 )
      mqtt_client.publish(String(topic+"/HUMI").c_str(), String("{\"Humidity \":\"")+String(hmdt).c_str()+String("\",")+String("\"Time \":\"")+String(dateTime)+String("\"}")) ;
  }
}
